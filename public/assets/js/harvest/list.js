...
$("#export_btncomp").on('click', function(event) {
    // $.ajax({
    //     url:'get_harvest_table_data',
    //     type:'post',
    //     data:'date_range=' + $("#reservation").val(),
    //     async:false,
    //     success:function(res)
    //     {
    //         if(res == "") return;
            var res = harvest_table.rows().data();
            convertToCSVCOMP(res).then(function(result){
                let filename = $("#reservation").val();
...
//MGMTCSV
var convertToCSVCOMP = (objArray) => {

return new Promise(function(next_operation){

var array = typeof objArray != 'object' ? JSON.parse(objArray) : objArray;
var str = "id,harvest_batch_idkk,Total Weight,Total_Count,Unit Of Weight,Flower Room Location,Strain,License,Creation Date\r\n";
...
//line += array[i].ant_dry_weighton + ',';
//line += array[i].ant_dry_weightgr + ',';
line += array[i].created_at + ',';
var sub_array = array[i].items;
var sub_result = ' ,Plant Tag,,Weight\r\n';
...
var row_details_format = (d) => {
    ...
    html += "</table>";
    html += "<h3>Fresh Item</h3>"
    data = d.fresh_item
    html += '<table class="table table-bordered" cellpadding="5" cellspacing="0" border="0" style="padding-left:50px;">';
    html += '<tr>';
    html += '<td>Weight</td><td>Metrc</td></tr>'
    if(data == null)
    {
    ...
    if(data.length == 0)
    {
        html += "<tr><td colspan=2 style='text-align:center'>No Fresh Item</td></tr>"
    }
    for(var i = 0; i < data.length; i ++)
    {
        html += '<tr style="font-size: small;">';
    ...
    if(data.length == 0)
    {
        html += "<tr><td colspan=2 style='text-align:center'>No Waist Item</td></tr>"
    }
    for(var i = 0; i < data.length; i ++)
    {
        html += '<tr style="font-size: small;">';
            ...
            "ajax": {
               ...
               //       json[i].ant_dry_weighton   = json[i].ant_dry_weightlbs * 16;
                ...
                //      json[i].ant_dry_weightgr = json[i].ant_dry_weightgr.toFixed(2);
                      ...
                      json[i].waist_btn = '<button class="btn btn-info btn-xs btn-edit waist_btn">Waste</button>'
                      json[i].fresh_btn    = '<button class="btn btn-info btn-xs btn-edit fresh_btn">Fresh</button>'
                      console.log(json[i].waist_item)
                      if(json[i].waist_item.length == 0)
                      {
                        json[i].dry_btn    = '<button class="btn btn-danger btn-xs btn-edit dry_btn" disabled>Not Ready</button>';
                      }
                      else
                      {
                        json[i].dry_btn    = '<button class="btn btn-info btn-xs btn-edit dry_btn">Ready -> HD</button>';
                      }
                    }
                    return json;
                  }
...
$('#harvest_table tbody').on('click', '.barcode_btn', function () {
    $('#modal_barcode_question').modal('show')
    
    let tr = $(this).closest('tr');
    let row = harvest_table.row( tr );
    sel_harvest_id = row.data().id;
    sel_harvest_id
...
$('.questionBtn').click(function(){
    let count = $('#question').val()
    
    if(isNaN(count))
    {
        alert('How many items')
        return false
    }

    $('#modal_barcode_question').modal('hide')
    $.ajax({
        data:'id=' + sel_harvest_id+'&count='+count,
...
$('#harvest_table tbody').on('click', '.statistic_btn', function () {
    let tr = $(this).closest('tr');
    let row = harvest_table.row( tr );
    let harvest_id = row.data().id;
    $.ajax({
        url:'_check_room_available',
        data:'id='+harvest_id,
        async:false,
        type:'post',
        success:(res) => {
            if(res == '1')
            {
                location.href="statistic?id=" + harvest_id
            }
            else
            {
                alert('No Statistics available for this harvest')
            }
        }
    })
...
$('#harvest_table tbody').on('click', '.csv_btncomp', function () {
    var tr = $(this).closest('tr');
    var row = harvest_table.row( tr );
    var data = Array();
    data[0] = row.data(); 
    
    convertToCSVCOMP(data).then(function(result){
        let filename = data[0].harvest_batch_id + data[0].created_at;
        exportCSVfile(filename,result);
    });
});
