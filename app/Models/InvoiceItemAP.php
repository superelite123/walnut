...
class InvoiceItemAP extends Model
{
    ...
    protected $fillable = ['invoice_id','strain','p_type','unit_price',
                           'qty','discount','e_discount','discount_type','discount_id',
                           'tax','tax_note','units'];
    ...
    {
        return number_format((float)$this->BasePrice - $this->discount - $this->e_discount, 2, '.', '');
    }

    public function getCPUAttribute()
    {
        $units = $this->units <= 0?1:$this->units;
        return number_format((float)$this->BasePrice / $units, 2, '.', '');
    }
    ...
    {
        return number_format((float)$this->e_discount / $this->qty, 2, '.', '');
    }
    ...
    {
        return number_format((float)$this->unit_price - $this->DividedDiscount - $this->DividedEDiscount, 2, '.', '');
    }
